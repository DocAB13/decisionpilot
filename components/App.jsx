...
const C = {
  ...
  accent: "#7C6AF7", accentGlow: "#7C6AF733", gold: "#F0B429", goldGlow: "#F0B42920",
  text: "#F0EFF8", muted: "#8887A4", success: "#34D399", tag: "#1E1E30",
};

function amz(keywords) {
  return `/go?url=${encodeURIComponent(`https://www.amazon.com/s?k=${encodeURIComponent(keywords)}`)}`;
}
  ...
  options: [
    {
      ...
      options: [
        ...
        {
          label: "🏛️ Culture & City", id: "culture", question: "Which region appeals to you?", emoji: "🗺️",
          options: [
    ...
    {
      label: "📱 Smartphone", id: "phone", question: "What matters most to you?", emoji: "📱",
      options: [
        { label: "📸 Best Camera", id: "phone_camera", result: { title: "Top Camera Phones 2026", description: "If photos and videos are your priority, these are the champions.", picks: [
          { name: "iPhone 16 Pro Max", tag: "Best Overall", desc: "Cinematic mode, ProRes video, titanium build.", link: amz("iPhone 16 Pro Max") },
          { name: "Google Pixel 9 Pro", tag: "Best AI Camera", desc: "Google's AI magic makes every photo look professional.", link: amz("Google Pixel 9 Pro") },
          { name: "Samsung Galaxy S25 Ultra", tag: "Most Versatile", desc: "200MP sensor, 10x optical zoom, S Pen included.", link: amz("Samsung Galaxy S25 Ultra") },
        ]}},
        { label: "🔋 Best Battery", id: "phone_battery", result: { title: "All-Day Battery Champions", description: "Never worry about charging again.", picks: [
        ...
        { label: "💰 Best Value", id: "phone_value", result: { title: "Premium Features, Smart Price", description: "You don't need $1,000+ for an excellent phone.", picks: [
    ...
    {
      label: "🚗 Car", id: "car", question: "What type of car are you looking for?", emoji: "🚗",
      options: [
    ...
    },
    {
      ...
      options: [
        { label: "🚀 Switch Jobs", id: "career_switch", result: { title: "How to Navigate a Job Switch", description: "A structured approach to making the right career move.", picks: [
...
function ResultCard({ pick, index }) {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${c}33`, borderRadius: 16, padding: "20px 22px", marginBottom: 12, boxShadow: `0 0 24px ${c}11`, transition: "transform 0.15s" }}
...
function ChatScreen({ onBack }) {
  const [messages, setMessages] = useState([{ role: "assistant", content: "Hi! I'm your DecisionPilot. Tell me about a decision you're facing — vacation, car, phone, career, or anything else. I'll ask a few questions and give you a personalized recommendation. 🧭" }]);
